(* Main - Day 20 Exercise 05
 * Tests for incomplete combustion
 *)

let () =
  print_endline "=== Day 20 Ex05: Testing Incomplete Combustion ===";
  print_endline "";
  ...
  let sample_count = min 8 (List.length propane_results) in
  List.iter (fun (o2, products) ->
    Printf.printf "With %d O2: " o2;
    let product_strs = List.map (fun (mol, coeff) ->
      if coeff = 1 then mol#to_string
  ...
  incomplete_butane#balance;
  print_endline ("Butane complete combustion: " ^ incomplete_butane#to_string);
  print_endline "";
